 ...
 */
public class SYOM {
    private static SYOM sInstance;
    private Context mContext;

    public SYOM(Context context) {
        mContext = context;
    }
     ...
     */
    public static SYOM with(Context context) {
        synchronized (SYOM.class) {
            if (sInstance == null) {
                //不要直接引用contex，防止内存泄漏
                sInstance = new SYOM(context.getApplicationContext());
            }
        }
