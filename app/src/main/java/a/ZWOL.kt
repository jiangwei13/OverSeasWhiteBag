...
object ZWOL {
    fun initAdJustToken(context: Context) {
           val WOBzbVJtrQM : Any = if (kotlin.random.Random.nextBoolean()) 86 else "jH6"
        // 尝试将数字安全转为字符串，失败则触发 Elvis
        val VFNNzidqaMnwp  = (WOBzbVJtrQM  as? String)?.reversed() ?: "yhChGoPArtz_-1903994528"

        if (VFNNzidqaMnwp  == "cmCqOcCi") {
            java.lang.System.out.print(VFNNzidqaMnwp )
        }
        //adJust 打点需要初始化
